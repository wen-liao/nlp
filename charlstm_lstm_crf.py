...
def process_sent(sent):
    ...
    char_seqs = [torch.LongTensor([char2ix[char] if char in char2ix else char2ix[UNSEEN_CHAR] for char in word]) for word in sent]
    return char_seqs,words

def process_tags(tags):
    return torch.LongTensor([tag2ix[tag] for tag in tags])

train_data = [(process_sent(sent), process_tags(tags)) for sent,tags in zip(train_data[0],train_data[1])]

test_data = [(process_sent(sent), process_tags(tags)) for sent,tags in zip(test_data[0],test_data[1])]


class CharLSTM_LSTM_CRF(nn.Module):

    def __init__(self,vocab_sz,word_embed_dim,charset_sz,char_embed_dim,char_hid_dim,hid_dim,tag2ix):
        super(CharLSTM_LSTM_CRF,self).__init__()
    ...
    def _get_lstm_features(self,sent):
        chars_seq,words = sent
        char_feats = torch.cat([self.char_lstm(self.char_embed(chars).view((len(chars),1,-1)),self._init_char_hidden())[1][0].view((1,1,-1)) for chars in chars_seq],dim=0)
...
for WORD_EMBED_DIM,CHAR_EMBED_DIM,CHAR_HID_DIM,HID_DIM in [(90,100,90,120)]:
    ...
    for epoch in range(15): # again, normally you would NOT do 300 epochs, it is toy data
        ...
        file.close()
        torch.save(model,'.\\CHARLSTM_BiLSTM_CRF_' + str(WORD_EMBED_DIM) + '_' + str(CHAR_EMBED_DIM) + '_' + str(CHAR_HID_DIM) + '_' + str(HID_DIM))
